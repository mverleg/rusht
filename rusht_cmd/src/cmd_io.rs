...
pub fn read(namespace: String) -> TaskStack {
    ...
    match serde_json::from_reader::<_, TaskStack>(reader) {
        Ok(tasks) => {
            debug!("successfully read {} commands from '{}'",
                tasks.len(), pth.to_string_lossy());
            tasks
        },
        Err(err) => {
            fail(&format!("failed to parse commands in '{}', error: {}",
                          pth.to_string_lossy(), err))
        },
    }
}
...
pub fn write(namespace: String, tasks: &TaskStack) {
    ...
    if tasks.is_empty() {
        if pth.exists() {
            debug!("commands stack is empty, deleting commands file at '{}'",
                pth.to_string_lossy());
            if let Err(err) = remove_file(&pth) {
                fail(&format!("failed to remove commands in '{}', error: {}",
                              pth.to_string_lossy(), err));
            }
        } else {
            debug!("commands stack is empty, there is no commands file at '{}', doing nothing",
                pth.to_string_lossy());
        }
    } else {
        let mut writer = BufWriter::new(open_file(&pth, true));
        if let Err(err) = serde_json::to_writer_pretty(&mut writer, tasks) {
            fail(&format!("failed to write commands in {}, error: {}",
                          pth.to_string_lossy(), err));
        }
        writer.write(&[b'\n']);
        debug!("wrote updated commands file with {} commands to '{}'",
            tasks.len(), pth.to_string_lossy());
    }
}
...
fn make_app_dir() -> PathBuf {
    let mut pth = match dirs::cache_dir() {
        Some(pth) => pth,
        None => fail("failed to rusht_find app data directory"),
    };
    pth.push("cmdstack");
    if let Err(err) = create_dir_all(&pth) {
        fail(format!("failed to create directory {}, error {}",
                     pth.to_string_lossy(), err))
    }
    pth
}
...
fn make_filename(namespace: String) -> String {
    ...
    format!("cmd_stack_{}_v{}.json", namespace.to_lowercase(), DATA_VERSION)
}
...
fn open_file(pth: &Path, write: bool) -> File {
    ...
    match opts.open(pth) {
        Ok(file) => file,
        Err(err) => {
            fail(&format!("failed to open commands file at '{}' with options {:?}, error {}",
                          pth.to_string_lossy(), &opts, err));
        }
    }
}
...
mod tests {
    ...
    fn default_stack_pth() {
        assert_eq!(stack_pth("".to_owned()).file_name().unwrap(),
                   format!("cmd_stack_v{}.json", DATA_VERSION).as_str());
    }

    #[test]
    fn namespaced_stack_pth() {
        assert_eq!(stack_pth("1".to_owned()).file_name().unwrap(),
