...
#[structopt(name = "cmadd", about = "Add a command to be executed to the stack. See also cmdo, cmlist, cmdrop")]
pub struct AddArgs {
    ...
    pub quiet: bool,
    #[structopt(short = "e", long, help = "Add command at the end (last) instead of as the next")]
    pub end: bool,
    #[structopt(short = "f", long, help = "Do not check that the command is known")]
    pub skip_validation: bool,
    #[structopt(short = "l", long, help = "Add command for each line of stdin, replacing '{}' by the line")]
    pub lines: bool,
    #[structopt(short = "L", long, help = "Like --lines, but use given replacement placeholder instead of '{}'", conflicts_with="lines")]
...
fn task_from_template(cmd: &[String], input: &str, templ: &str) -> PendingTask {
    PendingTask::new_split(cmd.iter()
        .map(|part| part.replace(templ, input))
        .collect())
}
